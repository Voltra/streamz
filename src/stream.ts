...
export class Stream<T> implements Iterable<T> {
	 ...
	 */
	private constructor(protected gen: Generator<T>) {}
	...
	static splitBy(
		...
		const predicate: Predicate<string> = removeEmptyStrings
			? (str) => str !== ""
			: () => true;
	...
	static range({
		...
		step = 1,
	}: Partial<RangeOptions> = {}) {
		const generatorFactory = function* () {
			if (end !== null) {
				let st = step;
				const cmp: BiFn<boolean, number, number> =
					start < end
						? (lhs, rhs) => lhs < rhs
						: (lhs, rhs) => lhs > rhs;
	...
	filter(predicate: Predicate<T>) {
		return this.pipe<T>(function* (source: Generator<T>) {
	...
	filterNot(predicate: Predicate<T>) {
		const negation = (value: T) => !predicate(value);
	...
	map<U>(mapper: Mapper<T, U>) {
		return this.pipe(function* (source: Generator<T>) {
			for (const value of source) {
				yield mapper(value);
			}
		});
	...
	flatten<E>(this: Stream<Either<E, Iterable<E>>>): Stream<E> {
		return this.pipe(function* (source: Generator<Either<E, Iterable<E>>>) {
			for (const value of source) {
				if (isIterable<E>(value)) {
					yield* value;
				} else {
					yield value;
				}
	 ...
	 */
	mapFlattened<E, U>(
		this: Stream<Either<E, Iterable<E>>>,
		mapper: Mapper<E, U>
	): Stream<U> {
	 ...
	 * @tag [OPERATOR: Stateless Operation]
	 */
	asInstanceOf<U>(clazz: Class<U>) {
		return this.instanceOf(clazz) as Stream<U>;
	}
	 ...
	 * @tag [OPERATOR: Stateless Operation]
	 */
	notNull(this: Stream<Nullable<T>>) {
		return this.filter((value) => value !== null) as Stream<T>;
	}
	 ...
	 * @tag [OPERATOR: Stateless Operation]
	 */
	notNullish(this: Stream<Maybe<T>>) {
		return this.filterNot(isNullish) as Stream<T>;
	}
	 ...
	 * @tag [OPERATOR: Stateless Operation]
	 */
	skipUntil(predicate: Predicate<T>) {
	 ...
	 * @tag [OPERATOR: Stateless Operation]
	 */
	takeUntil(predicate: Predicate<T>) {
	 ...
	 * @tag [OPERATOR: Stateless Operation]
	 */
	substream(startIndex: number, maxEndIndex: number) {
	 ...
	 * @tag [OPERATOR: Stateful Operation]
	 */
	sortBy<U>(idExtractor: Mapper<T, U>) {
	 ...
	 * @tag [OPERATOR: Stateful Operation]
	 */
	sortByDescending<U>(idExtractor: Mapper<T, U>) {
	 ...
	 * @tag [OPERATOR: Stateful Operation]
	 */
	uniqueBy<U>(idExtractor: Mapper<T, U>) {
		return this.pipe(function* (source: Generator<T>) {
			const yielded = new Set<U>();
	 ...
	 * @tag [TERMINATOR]
	 */
	reduce<Acc>(reducer: Reducer<Acc, T>, init: Nullable<Acc> = null) {
		let acc: Nullable<Acc> = init;
		let firstDone = acc !== null;

		for (const value of this) {
			if (!firstDone) {
				acc = value as unknown as Acc;
				firstDone = true;
			} else {
				acc = reducer(acc as Acc, value);
			}
		}
	...
	associateBy<Key, Value>(
		keyExtractor: Mapper<T, Key>,
		valueExtractor: Mapper<T, Value>
	) {
		return this.reduce(function (map, item) {
			map.set(keyExtractor(item), valueExtractor(item));
			return map;
		}, new Map<Key, Value>()) as Map<Key, Value>;
	}
	 ...
	 * @tag [TERMINATOR]
	 */
	associate<Key, Value>(entryFactory: Mapper<T, [Key, Value]>) {
		return this.reduce(function (map, item) {
	 ...
	 * @tag [TERMINATOR]
	 */
	associateUnzip<Key, Value>(
		this: Stream<[Key, Value]>,
		keyExtractor: Mapper<[Key, Value], Key> = KeyGen.entries,
		valueExtractor: Mapper<[Key, Value], Value> = ValueGen.entries
	) {
	 ...
	 * @tag [TERMINATOR]
	 */
	count(predicate: Predicate<T> = () => true) {
	 ...
	 * @tag [TERMINATOR]
	 */
	firstOrNull(predicate: Predicate<T> = () => true) {
		for (const value of this) {
			if (predicate(value)) {
				return value;
			}
	 ...
	 * @tag [TERMINATOR]
	 */
	firstOr<U>(defaultValue: U, predicate: Predicate<T> = () => true) {
		return this.firstOrNull(predicate) ?? defaultValue;
	}
	 ...
	 * @tag [TERMINATOR]
	 */
	first(predicate: Predicate<T> = () => true) {
	 ...
	 * @tag [TERMINATOR]
	 */
	atIndexOr<U>(index: number, defaultValue: U) {
		return this.atIndexOrNull(index) ?? defaultValue;
	}
	 ...
	 * @tag [TERMINATOR]
	 */
	groupBy<Key>(keyExtractor: Mapper<T, Key>) {
		return this.reduce(function (map, item) {
	 ...
	 * @tag [TERMINATOR]
	 */
	indexOfFirst(predicate: Predicate<T>) {
		let cursor = 0;
	 ...
	 * @tag [TERMINATOR]
	 */
	indexOfLast(predicate: Predicate<T>) {
		let cursor = 0;
	...
	join({
		...
		suffix = "",
		separator = "",
		stringifier = (item) => `${item}`,
	}: Partial<JoinOptions<T>> = {}) {
		let ret = prefix;
		let hasElems = false;
	 ...
	 * @tag [TERMINATOR]
	 */
	joinToString(joinOptions: Partial<JoinOptions<T>> = {}) {
	 ...
	 * @tag [TERMINATOR]
	 */
	lastOrNull(predicate: Predicate<T> = () => true) {
		let last: Nullable<T> = null;
	 ...
	 * @tag [TERMINATOR]
	 */
	lastOr<U>(defaultValue: U, predicate: Predicate<T> = () => true) {
		return this.lastOrNull(predicate) ?? defaultValue;
	}
	 ...
	 * @tag [TERMINATOR]
	 */
	last(predicate: Predicate<T> = () => true) {
	 ...
	 * @tag [TERMINATOR]
	 */
	maxBy(keyFactory: Mapper<T, number>) {
	 ...
	 * @tag [TERMINATOR]
	 */
	minBy(keyFactory: Mapper<T, number>) {
	 ...
	 * @tag [TERMINATOR]
	 */
	sumBy(valueExtractor: Mapper<T, number>, init: number = 0) {
	 ...
	 * @tag [TERMINATOR]
	 */
	sum(this: Stream<number>, init: number = 0) {
	 ...
	 * @tag [TERMINATOR]
	 */
	singleOrNull(predicate: Predicate<T> = () => true) {
	 ...
	 * @tag [TERMINATOR]
	 */
	singleOr<U>(defaultValue: U, predicate: Predicate<T> = () => true) {
		return this.singleOrNull(predicate) ?? defaultValue;
	}
	 ...
	 * @tag [TERMINATOR]
	 */
	single(predicate: Predicate<T> = () => true) {
	 ...
	 * @tag [TERMINATOR]
	 */
	unzip<A, B>(this: Stream<[A, B]>) {
		return this.reduce(
			function (acc, [lhs, rhs]) {
				acc[0].push(lhs);
				acc[1].push(rhs);
				return acc;
			},
	 ...
	 * @tag [TERMINATOR]
	 */
	toObject<Key extends PropertyKey, Value>(
		keyExtractor: Mapper<T, Key>,
		valueExtractor: Mapper<T, Value>,
		initialObject = {} as Record<Key, Value>
	) {
	 ...
	 * @tag [TERMINATOR]
	 */
	toObjectBy<Key extends PropertyKey>(
		keyExtractor: Mapper<T, Key>,
		initialObject = {} as Record<Key, T>
	) {
		return this.toObject<Key, T>(keyExtractor, (x) => x, initialObject);
	}
	...
	toMap<Key, Value>(
		keyExtractor: Mapper<T, Key>,
		valueExtractor: Mapper<T, Value>,
		initialMap: Map<Key, Value> = new Map()
	) {
		return this.reduce(function (map, item) {
			const key = keyExtractor(item);
